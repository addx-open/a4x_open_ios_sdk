...
class A4xVideoPushMessageView: UIView {
    ...
    private var messageContentModle : A4xVideoMessageModel?
    ...
    func showMessageContent(message : A4xVideoMessageModel) -> Bool{
        ...
        if !showContentMessage && !showCountMessage { //如果图文显示和数量显示  都不显示，来消息收展示消息
            ...
            // A4xVideoPushMessageView.messagePadding
            messageContentView.frame = CGRect(x: UIScreen.main.bounds.midX - CGFloat(messageWidth / 2), y: -height, width: CGFloat(messageWidth), height: height)
            ...
            UIView.animate(withDuration: A4xVideoPushMessageView.animailDuration, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0, options: UIView.AnimationOptions.allowAnimatedContent) { [weak self] in
                ...
                // A4xVideoPushMessageView.messagePadding
                self?.messageContentView.frame = CGRect(x: UIScreen.main.bounds.midX - CGFloat(messageWidth / 2),  y: top, width: CGFloat(messageWidth), height: height)
                
            } completion: { (f) in
                NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(self.hiddenMessageCountent), object: nil)
                let time : TimeInterval = A4xVideoPushManager.shared.config?.pushShowTime ?? 5
        ...
        if !showCountMessage {
            ...
            UIView.animate(withDuration: A4xVideoPushMessageView.animailDuration, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0, options: UIView.AnimationOptions.allowAnimatedContent) { [weak self] in
                ...
                let rect = CGRect(x: self?.messageContentView.frame.minX ?? 0,
                                  y: (self?.messageContentView.frame.maxY ?? 0) + (A4xVideoPushMessageView.messagePadding),
                ...
                let time : TimeInterval = A4xVideoPushManager.shared.config?.pushShowTime ?? 15
    ...
    @objc private func oneClick(tap: UITapGestureRecognizer) {
        ...
        if self.messageContentView.frame.contains(point) {
            ...
            if case .moveBar = type {
                ...
                A4xVideoPushManager.shared.config?.messageContentClick?(type ,messageContentModle)
    ...
    func updateCountViewFrame()  {
        if self.showContentMessage && showCountMessage {
            ...
            let rect = CGRect(x: self.messageContentView.frame.minX,
                              y: (self.messageContentView.frame.maxY) + A4xVideoPushMessageView.messagePadding,
                                                  width: self.messageCountView.frame.width,
                                                  height: messageCountView.frame.height)
            self.messageCountView.frame = rect
            ...
            let rect = CGRect(x: self.messageContentView.frame.minX,
                              y: top + A4xVideoPushMessageView.messagePadding ,
                                                  width: self.messageCountView.frame.width,
                                                  height: messageCountView.frame.height)
            self.messageCountView.frame = rect
